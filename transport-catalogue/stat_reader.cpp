...
void ParseAndPrintStat(const TransportCatalogue& transport_catalogue, std::string_view request,
    ...
    if (request[0] == 'B') {
        ...
        auto [stops_count, unique_stops_count, route_length, curvature] = transport_catalogue.GetBusInfo(bus_name);
        if (!stops_count) {
            output << "Bus "s << bus_name << ": not found\n"s;
            return;
        }
        output << "Bus "s << bus_name << ": "s << stops_count << " stops on route, "s;
        output << unique_stops_count << " unique stops, "s << route_length << " route length, "s;
        output << std::setprecision(6) << curvature << " curvature\n"s;
    }
    else if (request[0] == 'S') {
